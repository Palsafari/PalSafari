      ...
      >
        {hikingPackages.map((hikingPackage) => (
          <SwiperSlide key={hikingPackage.id}>
            <Product
              pckg={hikingPackage}
              badge={hikingPackage.badge || "AFFORDABLE"}
              lead_img={hikingPackage.lead_image[0] || assets.lead_img}
              num_days={hikingPackage.num_days || 1}
              country={hikingPackage.country || "International"}
              title={
              ...
              pal_price={
                hikingPackage.high_season.includes(
                  ...
                  ? Math.ceil(
                      (hikingPackage.high_season_price || 200) *
                        ((100 - (hikingPackage.discount.percentage || 5)) / 100)
                    )
                  : Math.ceil(
                      (hikingPackage.low_season_price || 150) *
                        ((100 - (hikingPackage.discount.percentage || 5)) / 100)
                    )
              }
